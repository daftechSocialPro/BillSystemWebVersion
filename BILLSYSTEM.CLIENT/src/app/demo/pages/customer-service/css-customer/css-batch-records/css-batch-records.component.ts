...
export class CssBatchRecordsComponent implements OnInit {
  ...
  saveSelectedBatchValues() {
    ...
    const selectedCustIds = selectedCustomers.map((item)=>{
      return (item.custId)
    })

    const customerBatch : ICustomerBatchDto={
      selectedCustomerIds :selectedCustIds,
      changeByName:this.selectedBatchValue,
      changedValue:this.selectedBatchValue2
    }


    console.log("sdjkfds",customerBatch)


    

    this.customerService.changeValueByBatch(customerBatch).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  filter() {
    var value=this.selectedBatchValue;
    if (value == 'BankCode') {
      this.getBanks();
    }
